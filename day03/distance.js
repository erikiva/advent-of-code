...
const findIntersections = (positions1, cable2) => {
 ...
 for (let i = 0; i < cable2.length; i++){
  ...
  for (let i = 0; i < possitions2.length; i++){
    ...
    if (positions1[current.join('_')] == 1){
      //console.log(found, current);
      if (Math.abs(current[0]) + Math.abs(current[1]) < bestIntersection ){
        bestIntersection = Math.abs(current[0]) + Math.abs(current[1]);
      }  
    }
  }
 }
