...
pub fn input_generator(input: &str) -> Bingo {
  ...
  let moves = input.next().unwrap().split(",").map(|m| m.parse().unwrap()).collect();
  let boards: Vec<Board> = input.map(|b| {
    Board {
      rows: b.lines()
        .map(|l| l.trim().split_whitespace().map(|n| n.parse().unwrap()).collect())
        .collect(),
      rows_done: [0; 5],
      cols_done: [0; 5],
      winner: false
      ...
      for r in 0..boards[b].rows.len() {
        for c in 0..boards[b].rows[0].len(){
          ...
          if boards[b].rows_done[r] == 5 || boards[b].cols_done[c] == 5 {
            return  boards[b].rows.iter().map(|r| r.iter().filter(|c| **c != -1i32).sum::<i32>()).sum::<i32>() * moves[n];
          }
        }
      }
    }
...
use super::*;
  ...
  #[test]
  fn example1() {
    assert_eq!(solve_part1(&input_generator("7,4,9,5,11,17,23,2,0,14,21,24,10,16,13,6,15,25,12,22,18,20,8,19,3,26,1
  ...
  #[test]
  fn example2() {
    assert_eq!(solve_part2(&input_generator("7,4,9,5,11,17,23,2,0,14,21,24,10,16,13,6,15,25,12,22,18,20,8,19,3,26,1
