...
pub fn solve_part2(input: &Vec<Vec<char>>) -> u64 {
  let incompletes: Vec<Vec<char>> = input.into_iter().filter(|&line| {
    let value = process_line(line.clone());
    return value == 0;
  }).cloned().collect::<Vec<Vec<char>>>();

  let mut vals: Vec<u64> = incompletes.into_iter().map(find_missing).map(calculate_score).collect::<Vec<u64>>();
  vals.sort();
  let med = vals.len() / 2;
  return vals[med];
}
...
const INPUT: &str = r#"[({(<(())[]>[[{[]{<()<>>
  ...
  #[test]
  fn example_score1() {
    assert_eq!(calculate_score(vec!['}','}',']',']', ')','}', ')',']']), 288957);
  }
