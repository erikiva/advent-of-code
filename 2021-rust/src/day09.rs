...
pub fn is_valid(cave_map: &Vec<Vec<i32>>, next: &(i32,i32), current: i32, dimensions: &(usize, usize), visited: &HashMap<(usize, usize), bool>) -> bool {
  if next.0 < 0 || next.0 >= dimensions.0 as i32 || next.1 < 0 || next.1 >= dimensions.1 as i32 {
    return false;
  }
  let next_pos = (next.0 as usize, next.1 as usize);
  if let Some(_) = visited.get(&next_pos) {return false}
  ...
  while !queue.is_empty() {
    if let Some(current) = queue.pop_front() {
      for neighbour in neighbours {
        let new_point: (i32, i32) = (current.0 as i32 + neighbour.0, current.1 as i32 + neighbour.1);
        if is_valid(&cave_map.clone(),
              &new_point,
              cave_map[current.0 as usize][current.1 as usize].clone(),
              &dimensions, &visited) {
...
pub fn solve_part1(input: &Vec<Vec<i32>>) -> i32 {
  ...
  low_points.iter().map(|&(lp_r, lp_c)| input[lp_r][lp_c] + 1).sum()
