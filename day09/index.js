  ...
  getOperandValue(mode, index, list) {
    switch(mode) {
      ...
      case 1:
        return list[index] || 0;
      case 2:
        return list[this.base + list[index]] || 0;
    }
    ...
    while (true && this.instructions[index] != 0 && this.instructions[index] != 1000) {
      ...
      if (opcode[4] == 1 || opcode[4] == 2 || opcode[4] == 7 || opcode[4] == 8){
        ...
        let param = this.getOperandValue(opcode[2], index + 1, this.instructions);
          if (param > 0) {
            let position = this.getOutputPosition(opcode[1], index + 2);
            index = position;
            console.log(` instruction ${opcode[4]}:  ${param}  ->  ${position}`);
          } else {
            index = index + 3;
          }
      } else if (opcode[4] == 6) {
