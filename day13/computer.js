class Computer {
  ...
  getOpcode(num) {
    ...
    console.log(
      `pc=${this.pc} bp=${this.base} | ` + this.decodeOpCode(numArray)
    );
    return numArray;
  }
...
function* inputGenerator(arr) {
  if (typeof arr == "object" && arr.length > 0) {
    console.log("Going with array values");
    for (let i = 0; i < arr.length; i++) {
      yield arr[i];
    }
  } else {
    console.log("Going with random values");
    const joyStickValues = [-1, 0, 1];
    while (true) {
      yield joyStickValues[Math.floor(Math.random() * joyStickValues.length)];
    }
  }
}
