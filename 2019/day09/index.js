...
class Amplifier {
  ...
  getOpcode(num)  {
    ...
    //console.log(this.instructions.slice(this.pc, this.pc+this.getOpcodeLength(numArray)))
    console.log(`pc=${this.pc} bp=${this.base} | ` + this.decodeOpCode(numArray))
    return numArray;
  }
  ...
  getOperandValue(mode, index, list) {
    ...
    console.log("read", result, "from", this.getOutputPosition(mode, index))
    return result
  }
  ...
  calculateInstructions() {
    let index = this.pc;
    let output = 0;
    while (true && this.instructions[index] != 0 && this.instructions[index] != 1000) {
      this.pc = index;
      ...
      // console.log(`   main params - base: ${this.base} - index: ${index} - opcode: ${opcode.join('')} `);
      if (!this.opcodeIsValid(opcode)) {
        console.log("illegal instruction", opcode, "at", index)
        break
      }
      if (opcode[4] == 1 || opcode[4] == 2 || opcode[4] == 7 || opcode[4] == 8){
        let value1 = this.getOperandValue(opcode[2], index + 1, this.instructions);
        let value2 = this.getOperandValue(opcode[1], index + 2, this.instructions);
        let finalPosition = this.getOutputPosition(opcode[0], index + 3);
        // console.log(` instruction ${opcode[4]}:  ${value1} -  ${value2} ->  ${finalPosition}`);
        let result = 0;
        if (opcode[4] == 1) {
        ...
        let param = this.getOperandValue(opcode[2], index + 1, this.instructions);
          if (param != 0) {
            let position = this.getOperandValue(opcode[1], index + 2, this.instructions);
            index = position;
        ...
        let value = this.getOperandValue(opcode[2], index + 1, this.instructions);
        // console.log(` instruction ${opcode[4]}:  ${value}`);
        ...
        if (opcode[4] == 3){
          ...
          // console.log(`------ instruction ${opcode[4]}:  ${value} - ${position} ${this.instructions[index + 1]}`);
          this.instructions[position] = value;
...
const calculateKeycode = (data, input = [1]) => {
  ...
  console.log('input passed to program: ', input);
  while (resume) {
    resume = intcode.calculateInstructions();
    if (resume)
    outputs.push(intcode.output);
  }
  console.log('**** Final output ****', outputs.join());
